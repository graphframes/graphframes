...
class VertexRDDSuite extends AnyFunSuite with LocalSparkContext {
  ...
  test("leftJoin") {
    withSpark { sc =>
      ...
      assert(
        verts.leftJoin(evens) { (_, a, bOpt) => a - bOpt.getOrElse(0) }.collect().toSet ===
      ...
      assert(
        verts.leftJoin(evensRDD) { (_, a, bOpt) => a - bOpt.getOrElse(0) }.collect().toSet ===
          (0 to n by 2).map(x => (x.toLong, 0)).toSet ++ (1 to n by 2)
            .map(x => (x.toLong, x))
            .toSet)
    }
  ...
  test("leftJoin vertices with non-equal number of partitions") {
    withSpark { sc =>
      ...
      assert(vertexA.partitions.length != vertexB.partitions.length)
      val vertexC = vertexA.leftJoin(vertexB) { (_, old, newOpt) =>
  ...
  test("innerJoin") {
    withSpark { sc =>
      ...
      assert(
        verts.innerJoin(evens) { (_, a, b) => a - b }.collect().toSet ===
          (0 to n by 2).map(x => (x.toLong, 0)).toSet)
      // innerJoin with an RDD
      val evensRDD = evens.map(identity)
      assert(
        verts.innerJoin(evensRDD) { (_, a, b) => a - b }.collect().toSet ===
          (0 to n by 2).map(x => (x.toLong, 0)).toSet)
    }
  }
  ...
  test("innerJoin vertices with the non-equal number of partitions") {
    withSpark { sc =>
      ...
      val vertexB =
        VertexRDD(vertexA.filter(v => v._1 % 2 == 0).partitionBy(new HashPartitioner(3)))
      assert(vertexA.partitions.length != vertexB.partitions.length)
      val vertexC = vertexA.innerJoin(vertexB) { (_, old, newVal) =>
        old - newVal
      }
