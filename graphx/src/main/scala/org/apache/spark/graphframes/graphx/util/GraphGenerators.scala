...
object GraphGenerators extends Logging {
   ...
   * Leiser, and Grzegorz Czajkowski. 2010. Pregel: a system for large-scale graph processing.
   * SIGMOD '10.
   *
  ...
  def logNormalGraph(
    ...
    val vertices: RDD[(VertexId, Long)] =
      sc.parallelize(0 until numVertices, evalNumEParts).map { src =>
        (src.toLong, sampleLogNormal(mu, sigma, numVertices, seed = (seed1 ^ src).toLong).toLong)
      }
  ...
  def generateRandomEdges(
    ...
    Array.fill(numEdges) { Edge[Int](src.toLong, rand.nextInt(maxVertexId).toLong, 1) }
  }

  /**
   * Randomly samples from the given mean and standard deviation of the normal distribution. It
   * uses the formula `X = exp(mu+sigma*Z)` where `mu`, `sigma` are the mean, standard deviation
   * of the normal distribution and `Z ~ N(0, 1)`.
   *
   * @param mu
  ...
  /**
   * A random graph generator using the R-MAT model, proposed in "R-MAT: A Recursive Model for
  ...
  def rmatGraph(sc: SparkContext, requestedNumVertices: Int, numEdges: Int): Graph[Int, Int] = {
    ...
    val numVertices =
      math
        .round(math.pow(2.0, math.ceil(math.log(requestedNumVertices.toDouble) / math.log(2.0))))
        .toInt
    val numEdgesUpperBound =
      math.pow(2.0, 2 * ((math.log(numVertices.toDouble) / math.log(2.0)) - 1)).toInt
  ...
  /**
   * @param numVertices
   *   Specifies the total number of vertices in the graph (used to get the dimensions of the
   *   adjacency matrix
   */
  private def addEdge(numVertices: Int): Edge[Int] = {
    ...
    Edge[Int](src.toLong, dst.toLong, 1)
  }
  ...
  /**
   * Create `rows` by `cols` grid graph with each vertex connected to its row+1 and col+1
   * neighbors. Vertex ids are assigned in row major order.
   *
   * @param sc
   ...
   *
   * @return
   *   A graph containing vertices with the row and column ids as their attributes and edge values
   *   as 1.0.
   */
  def gridGraph(sc: SparkContext, rows: Int, cols: Int): Graph[(Int, Int), Double] = {
    // Convert row column address into vertex ids (row major order)
    def sub2ind(r: Int, c: Int): VertexId = (r * cols + c).toLong
    ...
    Graph[(Int, Int), Double](vertices, edges)
  } // end of gridGraph
  ...
  def starGraph(sc: SparkContext, nverts: Int): Graph[Int, Int] = {
    val edges: RDD[(VertexId, VertexId)] =
      sc.parallelize(1 until nverts).map(vid => (vid.toLong, 0L))
