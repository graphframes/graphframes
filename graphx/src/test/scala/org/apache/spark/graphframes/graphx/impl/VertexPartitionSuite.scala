...
class VertexPartitionSuite extends AnyFunSuite {

  test("isDefined, filter") {
    val vp = VertexPartition(Iterator((0L, 1), (1L, 1))).filter { (vid, _) => vid == 0 }
  ...
  test("map") {
    val vp = VertexPartition(Iterator((0L, 1), (1L, 1))).map { (_, _) => 2 }
    assert(vp(0) === 2)
  }

  test("diff") {
    val vp = VertexPartition(Iterator((0L, 1), (1L, 1), (2L, 1)))
    val vp2 = vp.filter { (vid, _) => vid <= 1 }
    val vp3a = vp.map { (_, _) => 2 }
  ...
  test("leftJoin") {
    val vp = VertexPartition(Iterator((0L, 1), (1L, 1), (2L, 1)))
    val vp2a = vp.filter { (vid, _) => vid <= 1 }.map { (_, _) => 2 }
    val vp2b = VertexPartition(vp2a.iterator)
    // leftJoin with same index
    val join1 = vp.leftJoin(vp2a) { (_, a, bOpt) => bOpt.getOrElse(a) }
    assert(join1.iterator.toSet === Set((0L, 2), (1L, 2), (2L, 1)))
    // leftJoin with different indexes
    val join2 = vp.leftJoin(vp2b) { (_, a, bOpt) => bOpt.getOrElse(a) }
    assert(join2.iterator.toSet === Set((0L, 2), (1L, 2), (2L, 1)))
    // leftJoin an iterator
    val join3 = vp.leftJoin(vp2a.iterator) { (_, a, bOpt) => bOpt.getOrElse(a) }
  ...
  test("serialization") {
    ...
    for (ser <- List(javaSer, kryoSer); s = ser.newInstance()) {
      val vpSer: VertexPartition[Int] = s.deserialize[VertexPartition[Int]](s.serialize(vp))
