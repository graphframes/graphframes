...
/**
 * Manages shipping vertex attributes to the edge partitions of an [[graphframes.graphx.EdgeRDD]].
 * Vertex attributes may be partially shipped to construct a triplet view with vertex attributes
 * on only one side, and they may be updated. An active vertex set may additionally be shipped to
 * the edge partitions. Be careful not to store a reference to `edges`, since it may be modified
 * when the attribute shipping level is upgraded.
 */
private[impl] class ReplicatedVertexView[VD: ClassTag, ED: ClassTag](
  ...
  /**
   * Return a new `ReplicatedVertexView` where edges are reversed and shipping levels are swapped
   * to match.
   */
  ...
  /**
   * Upgrade the shipping level in-place to the specified levels by shipping vertex attributes
   * from `vertices`. This operation modifies the `ReplicatedVertexView`, and callers can access
   * `edges` afterwards to obtain the upgraded view.
   */
  ...
  /**
   * Return a new `ReplicatedVertexView` where the `activeSet` in each edge partition contains
   * only vertex ids present in `actives`. This ships a vertex id to all edge partitions where it
   * is referenced, ignoring the attribute shipping level.
   */
  ...
  /**
   * Return a new `ReplicatedVertexView` where vertex attributes in edge partition are updated
   * using `updates`. This ships a vertex attribute only to the edge partitions where it is in the
