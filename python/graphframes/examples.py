...
class Graphs(object):
    ...
    def gridIsingModel(self, n, vStd=1.0, eStd=1.0):
        ...
        toIDudf = sql_functions.udf(lambda i, j: '{},{}'.format(i,j))

        # create the vertex DataFrame
        # create SQL expression for converting coordinates (i,j) to a string ID "i,j"
        vIDcol = toIDudf(sql_functions.col('i'), sql_functions.col('j'))
        # add random parameters generated from a normal distribution
        seed = 12345
        vertices = (coordinates.withColumn('id', vIDcol)
            .withColumn('a', sql_functions.randn(seed) * vStd))

        # create the edge DataFrame
        # create SQL expression for converting coordinates (i,j+1) and (i+1,j) to string IDs
        rightIDcol = toIDudf(sql_functions.col('i'), sql_functions.col('j') + 1)
        downIDcol = toIDudf(sql_functions.col('i') + 1, sql_functions.col('j'))
        horizontalEdges = (coordinates.filter(sql_functions.col('j') != n - 1)
            .select(vIDcol.alias('src'), rightIDcol.alias('dst')))
        verticalEdges = (coordinates.filter(sql_functions.col('i') != n - 1)
        ...
        edges = allEdges.withColumn('b', sql_functions.randn(seed + 1) * eStd)
