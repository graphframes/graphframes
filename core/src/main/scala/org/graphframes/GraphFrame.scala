  ...
  def toGraphX: Graph[Row, Row] = {
    if (hasIntegralIdType) {
      val vv = vertices.select(col(ID).cast(LongType), nestAsCol(vertices, ATTR)).rdd.map {
        ...
        case Row(null, _) => throw new IllegalArgumentException(s"Vertex ID cannot be null. Found null in column '$ID'.")
      ...
      val ee = edges
        ...
        .map {
          ...
          case Row(null, _, _) | Row(_, null, _) => throw new IllegalArgumentException(s"Edge '$SRC' and '$DST' cannot be null.")
          case _ => throw new GraphFramesUnreachableException()
        }
      Graph[Row, Row](vv, ee)
    } else {
