...
def test_all_paths(local_g: GraphFrame) -> None:
    ...
    paths = local_g.all_paths("name='A'", "name='C'")
    assert paths is not None
    ...
    paths_filtered = local_g.all_paths("name='A'", "name='C'", edge_filter="action!='follow'")
    ...
    paths_undirected = local_g.all_paths("name='A'", "name='C'", is_directed=False)
    assert paths_undirected.count() >= 1

    # max_path_length too short: no paths
    paths_short = local_g.all_paths("name='A'", "name='C'", max_path_length=1)
