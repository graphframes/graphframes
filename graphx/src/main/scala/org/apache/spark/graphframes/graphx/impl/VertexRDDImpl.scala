...
class VertexRDDImpl[VD] private[graphx] (
  ...
  override def mapValues[VD2: ClassTag](f: VD => VD2): VertexRDD[VD2] =
    this.mapVertexPartitions(_.map((_, attr) => f(attr)))
  ...
  override def minus(other: RDD[(VertexId, VD)]): VertexRDD[VD] = {
    minus(this.aggregateUsingIndex(other, (a: VD, _: VD) => a))
  }
  ...
  override def minus(other: VertexRDD[VD]): VertexRDD[VD] = {
    other match {
      ...
      case _ =>
        this.withPartitionsRDD[VD](partitionsRDD
          .zipPartitions(other.partitionBy(this.partitioner.get), preservesPartitioning = true) {
  ...
  override def diff(other: RDD[(VertexId, VD)]): VertexRDD[VD] = {
    diff(this.aggregateUsingIndex(other, (a: VD, _: VD) => a))
  }
  ...
  override def diff(other: VertexRDD[VD]): VertexRDD[VD] = {
    ...
    val newPartitionsRDD =
      partitionsRDD.zipPartitions(otherPartition, preservesPartitioning = true) {
        (thisIter, otherIter) =>
          val thisPart = thisIter.next()
          val otherPart = otherIter.next()
  ...
  override def leftJoin[VD2: ClassTag, VD3: ClassTag](other: RDD[(VertexId, VD2)])(
    ...
    other match {
      case other: VertexRDD[_] if this.partitioner == other.partitioner =>
        leftZipJoin(other)(f)
      case _ =>
        this.withPartitionsRDD[VD3](partitionsRDD
          .zipPartitions(other.partitionBy(this.partitioner.get), preservesPartitioning = true) {
  ...
  override def innerJoin[U: ClassTag, VD2: ClassTag](other: RDD[(VertexId, U)])(
    ...
    other match {
      ...
      case _ =>
        this.withPartitionsRDD(partitionsRDD
          .zipPartitions(other.partitionBy(this.partitioner.get), preservesPartitioning = true) {
