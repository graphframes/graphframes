...
import org.apache.spark.graphframes.graphx.impl._
...
abstract class Graph[VD: ClassTag, ED: ClassTag] protected () extends Serializable {
  ...
  /**
   * An RDD containing the edge triplets, which are edges along with the vertex data associated
   * with the adjacent vertices. The caller should use [[edges]] if the vertex data are not
   * needed, i.e. if only the edge data and adjacent vertex ids are needed.
   *
   * @return
   *   an RDD containing edge triplets
   *
   * @example
   *   This operation might be used to evaluate a graph coloring where we would like to check that
   *   both vertices are a different color.
   *   {{{
   * type Color = Int
   ...
   * enabling multiple queries to reuse the same construction process.
   */
  def cache(): Graph[VD, ED]

  /**
   * Mark this Graph for checkpointing. It will be saved to a file inside the checkpoint directory
   * set with SparkContext.setCheckpointDir() and all references to its parent RDDs will be
   * removed. It is strongly recommended that this Graph is persisted in memory, otherwise saving
   * it on a file will require recomputation.
   */
  def checkpoint(): Unit

  /**
   * Return whether this Graph has been checkpointed or not. This returns true iff both the
   * vertices RDD and edges RDD have been checkpointed.
   */
  def isCheckpointed: Boolean

  /**
   * Gets the name of the files to which this Graph was checkpointed. (The vertices RDD and edges
   * RDD are checkpointed separately.)
   */
   ...
   * iterative algorithms that modify the vertex attributes but reuse the edges. This method can
   * be used to uncache the vertex attributes of previous iterations once they are no longer
   * needed, improving GC performance.
   *
   * @param blocking
   *   Whether to block until all data is unpersisted (default: false)
   */
   ...
   *
   * @note
   *   The new graph has the same structure. As a consequence the underlying index structures can
   *   be reused.
   *
   * @param map
   ...
   *
   * @example
   *   We might use this operation to change the vertex values from one type to another to
   ...
   *
   * @note
   *   This graph is not changed and that the new graph has the same structure. As a consequence
   ...
   *
   * @note
   *   This does not change the structure of the graph or modify the values of this graph. As a
   *   consequence the underlying index structures can be reused.
   *
   * @param map
   *   a function that takes a partition id and an iterator over all the edges in the partition,
   *   and must return an iterator over the new values for each edge in the order of the input
   *   iterator
   *
   * @tparam ED2
   *   the new edge data type
   ...
   * attributes as well. If adjacent vertex values are not required, consider using `mapEdges`
   * instead.
   *
   * @note
   *   This does not change the structure of the graph or modify the values of this graph. As a
   ...
   *
   * @example
   *   This function might be used to initialize edge attributes based on the attributes
  ...
  def mapTriplets[ED2: ClassTag](map: EdgeTriplet[VD, ED] => ED2): Graph[VD, ED2] = {
    mapTriplets((_, iter) => iter.map(map), TripletFields.All)
  }
   ...
   *   the function from an edge object to a new edge value.
   * @param tripletFields
   *   which fields should be included in the edge triplet passed to the map function. If not all
   *   fields are needed, specifying this can improve performance.
   *
   * @tparam ED2
   *   the new edge data type
   *
  ...
  def mapTriplets[ED2: ClassTag](
    ...
    mapTriplets((_, iter) => iter.map(map), tripletFields)
  }
   ...
   * within a logical partition and should yield a new iterator over the new values of each edge
   * in the order in which they are provided. If adjacent vertex values are not required, consider
   ...
   *
   * @param epred
   *   the edge predicate, which takes a triplet and evaluates to true if the edge is to remain in
   *   the subgraph. Note that only edges where both vertices satisfy the vertex predicate are
   *   considered.
   *
   * @param vpred
   *   the vertex predicate, which takes a vertex object and evaluates to true if the vertex is to
   ...
   * @return
   *   a graph with vertices and edges that exist in both the current graph and `other`, with
   *   vertex and edge data from the current graph
   */
   ...
   *
   * @param merge
   *   the user-supplied commutative associative function to merge edge attributes for duplicate
   *   edges.
   *
   * @return
   ...
   * sent to either vertex in the edge. The `mergeMsg` function is then used to combine all
   ...
   * @param mergeMsg
   *   used to combine messages from `sendMsg` destined to the same vertex. This combiner should
   *   be commutative and associative.
   * @param tripletFields
   *   which fields should be included in the [[EdgeContext]] passed to the `sendMsg` function. If
   *   not all fields are needed, specifying this can improve performance.
   *
   * @example
   ...
   *
   * @note
   *   By expressing computation at the edge level we achieve maximum parallelism. This is one of
   *   the core functions in the Graph API that enables neighborhood level computation. For
   *   example this function can be used to count neighbors satisfying a predicate or implement
   *   PageRank.
   */
   ...
   *   not all fields are needed, specifying this can improve performance.
   * @param activeSetOpt
   *   an efficient way to run the aggregation on a subset of the edges if desired. This is done
   *   by specifying a set of "active" vertices and an edge direction. The `sendMsg` function will
   *   then run on only edges connected to active vertices by edges in the specified direction. If
   *   the direction is `In`, `sendMsg` will only be run on edges with destination in the active
   *   set. If the direction is `Out`, `sendMsg` will only be run on edges originating from
   *   vertices in the active set. If the direction is `Either`, `sendMsg` will be run on edges
   *   with *either* vertex in the active set. If the direction is `Both`, `sendMsg` will be run
   *   on edges with *both* vertices in the active set. The active set must have the same index as
   ...
   *
   * @param other
   *   the table to join with the vertices in the graph. The table should contain at most one
   *   entry for each vertex.
   * @param mapFunc
   *   the function used to compute the new vertex values. The map function is invoked for all
   *   vertices, even those that do not have a corresponding entry in the table.
   *
   * @example
   *   This function is used to update the vertices with new values based on external data. For
   *   example we could add the out-degree to each vertex record:
   *
   * {{{
...
object Graph {
   ...
   * @param uniqueEdges
   *   if multiple identical edges are found they are combined and the edge attribute is set to
   *   the sum. Otherwise duplicate edges are treated as separate. To enable `uniqueEdges`, a
   *   [[PartitionStrategy]] must be provided.
   ...
   *   `uniqueEdges` is `None`) and vertex attributes containing the total degree of each vertex.
   */
   ...
   *
   * @return
   *   a graph with edge attributes described by `edges` and vertices given by all vertices in
   *   `edges` with value `defaultValue`
   */
  def fromEdges[VD: ClassTag, ED: ClassTag](
    ...
    GraphImpl[VD, ED](edges, defaultValue, edgeStorageLevel, vertexStorageLevel)
  }

  /**
   * Construct a graph from a collection of vertices and edges with attributes. Duplicate vertices
   * are picked arbitrarily and vertices found in the edge collection but not in the input
   ...
   * @param defaultVertexAttr
   *   the default vertex attribute to use for vertices that are mentioned in edges but not in
   *   vertices
   * @param edgeStorageLevel
   *   the desired storage level at which to cache the edges if necessary
   * @param vertexStorageLevel
  ...
  def apply[VD: ClassTag, ED: ClassTag](
      ...
      edgeStorageLevel: StorageLevel = StorageLevel.MEMORY_ONLY,
      vertexStorageLevel: StorageLevel = StorageLevel.MEMORY_ONLY): Graph[VD, ED] = {
    GraphImpl[VD, ED](vertices, edges, defaultVertexAttr, edgeStorageLevel, vertexStorageLevel)
  }
   ...
   * convenience operations are defined in the [[GraphOps]] class which may be shared across
