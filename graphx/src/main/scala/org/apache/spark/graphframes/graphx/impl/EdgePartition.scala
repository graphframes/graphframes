...
/**
 * A collection of edges, along with referenced vertex attributes and an optional active vertex
 ...
 *
 * @param localSrcIds
 *   the local source vertex id of each edge as an index into `local2global` and `vertexAttrs`
 * @param localDstIds
 *   the local destination vertex id of each edge as an index into `local2global` and
 *   `vertexAttrs`
 * @param data
 *   the attribute associated with each edge
 * @param index
 *   a clustered index on source vertex id as a map from each global source vertex id to the
 *   offset in the edge arrays where the cluster for that vertex id begins
 * @param global2local
 *   a map from referenced vertex ids to local ids which index into vertexAttrs
 * @param local2global
 *   an array of global vertex ids where the offsets are local vertex ids
 * @param vertexAttrs
 *   an array of vertex attributes where the offsets are local vertex ids
 * @param activeSet
 *   an optional active vertex set for filtering computation on the edges
 */
private[graphx] class EdgePartition[
   ...
   *
   * Be careful not to keep references to the objects passed to `f`. To improve GC performance the
   * same object is re-used for each call.
   *
   * @param f
   *   a function from an edge to a new attribute
   * @tparam ED2
   ...
   *
   * @note
   *   The input iterator should return edge attributes in the order of the edges returned by
   *   `EdgePartition.iterator` and should return attributes equal to the number of edges.
   *
   * @param iter
   *   an iterator for the new attribute values
   * @tparam ED2
   *   the type of the new attribute
   * @return
   ...
   *
   * @param f
   *   an external state mutating user defined function.
   */
  ...
  /**
   * Merge all the edges with the same src and dest id into a single edge using the `merge`
   * function
   *
   * @param merge
   *   a commutative associative merge operation
   * @return
   ...
   *
   * Be careful not to keep references to the objects from this iterator. To improve GC
   * performance the same object is re-used in `next()`.
   *
   * @return
   ...
   * scanning all edges sequentially.
   *
   * @param sendMsg
   *   generates messages to neighboring vertices of an edge
   * @param mergeMsg
   *   the combiner applied to messages destined to the same vertex
   * @param tripletFields
   *   which triplet fields `sendMsg` uses
   * @param activeness
