...
The implementation follows the SybilRank algorithm published by Cao et al. at NSDI'12 ([Cao, Qiang, et al. "Aiding the detection of fake accounts in large scale social online services." 9th USENIX symposium on networked systems design and implementation (NSDI 12). 2012.](https://www.usenix.org/system/files/conference/nsdi12/nsdi12-final42_2.pdf)), with the weighted-graph extension of Boshmaf et al. The number of power iterations defaults to `ceil(iteration_multiplier * log10(N))`, where `N` is the number of vertices and `iteration_multiplier` is `1.0` by default.
