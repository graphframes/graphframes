...
class PageRankSuite extends AnyFunSuite with LocalSparkContext {

  def compareRanks(a: VertexRDD[Double], b: VertexRDD[Double]): Double = {
    a.leftJoin(b) { case (_, a, bOpt) => (a - bOpt.getOrElse(0.0)) * (a - bOpt.getOrElse(0.0)) }
      .map { case (_, error) => error }
      .sum()
  }

  def convergenceIterations[VD, ED](
      graph: Graph[VD, ED],
      resetProb: Double,
      tol: Double,
      errorTol: Double): (Int, Int) = {
    ...
    var staticGraphRankWithCheckPoint = graph.ops
      .staticPageRank(iterWithCheckPoint, resetProb, staticGraphRankPartial)
      .vertices
      .cache()
    while (compareRanks(staticGraphRankWithCheckPoint, dynamicRanks) >= errorTol) {
      iterWithCheckPoint += 1
      staticGraphRankWithCheckPoint = graph.ops
        .staticPageRank(iterWithCheckPoint, resetProb, staticGraphRankPartial)
        .vertices
        .cache()
    }
  ...
  test("Star PageRank") {
    withSpark { sc =>
      ...
      val notMatching = staticRanks
        .innerZipJoin(staticRanks2) { (_, pr1, pr2) =>
          if (pr1 != pr2) 1 else 0
        }
  ...
  test("Star PersonalPageRank") {
    withSpark { sc =>
      ...
      val staticRanks =
        starGraph.staticPersonalizedPageRank(0, numIter, resetProb).vertices.cache()

      val dynamicRanks = starGraph.personalizedPageRank(0, tol, resetProb).vertices.cache()
      assert(compareRanks(staticRanks, dynamicRanks) < errorTol)

      val parallelStaticRanks = starGraph
        .staticParallelPersonalizedPageRank(Array(0), numIter, resetProb)
        .mapVertices { case (_, vector) =>
          vector(0)
        }
        .vertices
        .cache()
      ...
      val otherStaticRanks =
        starGraph.staticPersonalizedPageRank(1, numIter, resetProb).vertices.cache()
      ...
      val otherParallelStaticRanks = starGraph
        .staticParallelPersonalizedPageRank(Array(0, 1), numIter, resetProb)
        .mapVertices { case (_, vector) =>
          vector(1)
        }
        .vertices
  ...
  test("Grid PageRank with checkpoint without intermediate normalization") {
    withSpark { sc =>
      ...
      val ranksA: Array[(VertexId, Double)] = PageRank
        .runWithOptions(gridGraph, numIter = 6, resetProb, srcId = None, normalized = true)
        .vertices
        .collect()
      ...
      val ranksB: Array[(VertexId, Double)] = PageRank
  ...
  test("Chain PersonalizedPageRank") {
    withSpark { sc =>
      ...
      val parallelStaticRanks = chain
        .staticParallelPersonalizedPageRank(Array(source), numIter, resetProb)
        .mapVertices { case (_, vector) =>
          vector(0)
        }
        .vertices
