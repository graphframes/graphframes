...
object SVDPlusPlus {
   ...
   * available here</a>.
   *
   * The prediction rule is rui = u + bu + bi + qi*(pu + |N(u)|^^-0.5^^*sum(y)), see the details
   * on page 6.
   *
   * @param edges
  ...
  def run(
    ...
    require(
      conf.maxIters > 0,
      "Maximum of iterations must be greater than 0," +
    ...
    val gJoinT3 = g
      .outerJoinVertices(t3) {
        (_: VertexId, vd: (Array[Double], Array[Double], Double, Double), msg: Option[Double]) =>
    ...
    (Graph[(Array[Double], Array[Double], Double, Double), Double](newVertices, g.edges), u)
  }
  ...
  private def materialize(g: Graph[_, _]): Unit = {
    val _ = (g.vertices.count(), g.edges.count())
    ()
  }
