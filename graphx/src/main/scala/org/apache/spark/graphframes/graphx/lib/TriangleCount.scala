 ...
 *
 * <ul> <li> Compute the set of neighbors for each vertex</li> <li> For each edge compute the
 * intersection of the sets and send the count to both vertices.</li> <li> Compute the sum at each
 * vertex and divide by two since each triangle is counted twice.</li> </ul>
 *
 * There are two implementations. The default `TriangleCount.run` implementation first removes
 * self cycles and canonicalizes the graph to ensure that the following conditions hold: <ul> <li>
 * There are no self edges</li> <li> All edges are oriented (src is greater than dst)</li> <li>
 * There are no duplicate edges</li> </ul> However, the canonicalization procedure is costly as it
 * requires repartitioning the graph. If the input data is already in "canonical form" with self
 * cycles removed then the `TriangleCount.runPreCanonicalized` should be used instead.
 *
 * {{{
...
object TriangleCount {
  ...
  def runPreCanonicalized[VD: ClassTag, ED: ClassTag](graph: Graph[VD, ED]): Graph[Int, ED] = {
    ...
    val setGraph: Graph[VertexSet, ED] = graph.outerJoinVertices(nbrSets) { (_, _, optSet) =>
      optSet.orNull
    }
