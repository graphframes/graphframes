...
class EdgePartitionSuite extends AnyFunSuite {
  ...
  test("map") {
    ...
    val edgePartition = builder.toEdgePartition
    assert(
      edgePartition.map(e => e.srcId + e.dstId).iterator.map(_.copy()).toList ===
  ...
  test("filter") {
    ...
    val edgePartition = builder.toEdgePartition
    val filtered = edgePartition.filter(et => et.srcId == 0, (vid, _) => vid == 0 || vid == 1)
  ...
  test("innerJoin") {
    ...
    assert(
      a.innerJoin(b) { (_, _, a, _) => a }.iterator.map(_.copy()).toList ===
  ...
  test("serialization") {
    ...
    for (ser <- List(javaSer, kryoSer); s = ser.newInstance()) {
      val aSer: EdgePartition[Int, Int] = s.deserialize[EdgePartition[Int, Int]](s.serialize(a))
