 ...
 * `VertexRDD`, use the [[org.apache.spark.graphframes.graphx.VertexRDD$ VertexRDD object]].
 *
 * Additionally, stores routing information to enable joining the vertex attributes with an
 * [[org.apache.spark.graphframes.graphx.EdgeRDD]].
 *
 * @example
...
abstract class VertexRDD[VD](sc: SparkContext, deps: Seq[Dependency[_]])
  ...
  /**
   * Restricts the vertex set to the set of vertices satisfying the given predicate. This
   * operation preserves the index for efficient joins with the original RDD, and it sets bits in
   * the bitmask rather than allocating new memory.
   *
   * It is declared and defined here to allow refining the return type from `RDD[(VertexId, VD)]`
   * to `VertexRDD[VD]`.
   *
   * @param pred
   *   the user defined predicate, which takes a tuple to conform to the `RDD[(VertexId, VD)]`
   *   interface
   */
   ...
   * @return
   *   a new VertexRDD with values obtained by applying `f` to each of the entries in the original
   *   VertexRDD
   */
   ...
   * @return
   *   a new VertexRDD with values obtained by applying `f` to each of the entries in the original
   *   VertexRDD. The resulting VertexRDD retains the same index.
   */
   ...
   * differing values; for values that are different, keeps the values from `other`. This is only
   * guaranteed to work if the VertexRDDs share a common ancestor.
   *
   * @param other
   ...
   * @param f
   *   the function mapping a vertex id and its attributes in this and the other vertex set to a
   ...
   * backed by a VertexRDD with the same index then the efficient [[leftZipJoin]] implementation
   * is used. The resulting VertexRDD contains an entry for each vertex in `this`. If `other` is
   * missing any vertex in this VertexRDD, `f` is passed `None`. If there are duplicates, the
   * vertex is picked arbitrarily.
   *
   * @tparam VD2
   *   the attribute type of the other VertexRDD
   * @tparam VD3
   ...
   *
   * @param other
   *   an RDD containing vertices to join. If there are multiple entries for the same vertex, one
   *   is picked arbitrarily. Use [[aggregateUsingIndex]] to merge multiple entries.
   * @param f
   *   the join function applied to corresponding values of `this` and `other`
   * @return
   *   a VertexRDD co-indexed with `this`, containing only vertices that appear in both `this` and
   *   `other`, with values supplied by `f`
   */
   ...
   *
   * @param messages
   *   an RDD containing messages to aggregate, where each message is a pair of its target vertex
   *   ID and the message data
   * @param reduceFunc
   *   the associative aggregation function for merging messages to the same vertex
   * @return
   *   a VertexRDD co-indexed with `this`, containing only vertices that received messages. For
   *   those vertices, their values are the result of applying `reduceFunc` to all received
   *   messages.
   */
   ...
   * Operations on the returned VertexRDD will preserve this storage level.
   *
   * This does not actually trigger a cache; to do this, call
   * [[org.apache.spark.graphframes.graphx.VertexRDD#cache]] on the returned VertexRDD.
   */
...
object VertexRDD {
   ...
   * [[EdgeRDD]]) from an RDD of vertex-attribute pairs. Duplicate entries are removed
   * arbitrarily.
   *
   * @tparam VD
  ...
  def apply[VD: ClassTag](
    ...
    VertexRDD(vertices, edges, defaultVal, (a, _) => a)
  }
