...
object PageRank extends Logging {
  ...
  private def runUpdate(
    ...
    } else { (_: VertexId, _: VertexId) =>
      resetProb
    }

    rankGraph.outerJoinVertices(rankUpdates) { (id, _, msgSumOpt) =>
  ...
  def runParallelPersonalizedPageRank[VD: ClassTag, ED: ClassTag](
    ...
    while (i < numIter) {
      ...
      rankGraph = rankGraph
        .outerJoinVertices(rankUpdates) { (vid, _, msgSumOpt) =>
  ...
  def runUntilConvergenceWithOptions[VD: ClassTag, ED: ClassTag](
    ...
    val pagerankGraph: Graph[(Double, Double), Double] = graph
      // Associate the degree with each vertex
      .outerJoinVertices(graph.outDegrees) { (_, _, deg) =>
        deg.getOrElse(0)
      }
      // Set the weight on the edges based on the degree
  ...
  private def normalizeRankSum(rankGraph: Graph[Double, Double], personalized: Boolean) = {
    ...
    if (personalized) {
      rankGraph.mapVertices((_, rank) => rank / rankSum)
    } else {
      val numVertices = rankGraph.numVertices
      val correctionFactor = numVertices.toDouble / rankSum
      rankGraph.mapVertices((_, rank) => rank * correctionFactor)
    }
  }
}
