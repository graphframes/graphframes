...
private[graphx] abstract class VertexPartitionBaseOps[
    VD: ClassTag,
    Self[X] <: VertexPartitionBase[X]: VertexPartitionBaseOpsConstructor](self: Self[VD])
  ...
  /**
   * Pass each vertex attribute along with the vertex id through a map function and retain the
   ...
   *
   * @param f
   *   the function applied to each vertex id and vertex attribute in the RDD
   *
   * @return
   *   a new VertexPartition with values obtained by applying `f` to each of the entries in the
   *   original VertexRDD. The resulting VertexPartition retains the same index.
   */
   ...
   *
   * @note
   *   The vertex set preserves the original index structure which means that the returned RDD can
   *   be easily joined with the original vertex-set. Furthermore, the filter only modifies the
   *   bitmap index and so no new values are allocated.
   */
   ...
   * keeps the values from `other`. The indices of `this` and `other` must be the same.
   */
  ...
  /**
   * Similar to innerJoin, but vertices from the left side that don't appear in iter will remain
   * in the partition, hidden by the bitmask.
   */
  ...
  def reindex(): Self[VD] = {
    ...
    val arbitraryMerge = (a: VD, _: VD) => a
