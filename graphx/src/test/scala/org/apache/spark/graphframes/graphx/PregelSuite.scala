...
class PregelSuite extends AnyFunSuite with LocalSparkContext {
  ...
  test("1 iteration") {
    withSpark { sc =>
      ...
      val result = Pregel(star, 0)(
        (_, attr, _) => attr,
        _ => Iterator.empty,
        (_: Int, _: Int) => throw new Exception("mergeMsg run unexpectedly"))
