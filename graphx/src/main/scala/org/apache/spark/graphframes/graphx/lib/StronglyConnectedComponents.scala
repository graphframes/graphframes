    ...
    while (sccWorkGraph.numVertices > 0 && iter < numIter) {
      ...
      do {
        ...
        val finalVertices = sccWorkGraph.vertices
          .filter { case (_, (_, isFinal)) => isFinal }
          .mapValues { (_, data) => data._1 }

        // write values to sccGraph
        sccGraph = sccGraph
          .outerJoinVertices(finalVertices) { (_, scc, opt) =>
            opt.getOrElse(scc)
          }
          .cache()
        ...
        sccWorkGraph = sccWorkGraph.subgraph(vpred = (_, data) => !data._2).cache()
      ...
      if (iter < numIter) {
        sccWorkGraph = sccWorkGraph.mapVertices { case (vid, (_, isFinal)) => (vid, isFinal) }
        ...
        sccWorkGraph = Pregel[(VertexId, Boolean), ED, VertexId](
          ...
          (_, myScc, neighborScc) => (math.min(myScc._1, neighborScc), myScc._2),
          e => {
