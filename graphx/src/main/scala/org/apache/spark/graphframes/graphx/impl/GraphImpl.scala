...
/**
 * An implementation of [[org.apache.spark.graphframes.graphx.Graph]] to support computation on
 * graphs.
 *
...
class GraphImpl[VD: ClassTag, ED: ClassTag] protected (
  ...
  @transient override lazy val triplets: RDD[EdgeTriplet[VD, ED]] = {
    ...
    replicatedVertexView.edges.partitionsRDD.mapPartitions(_.flatMap { case (_, part) =>
  ...
  override def groupEdges(merge: (ED, ED) => ED): Graph[VD, ED] = {
    val newEdges =
      replicatedVertexView.edges.mapEdgePartitions((_, part) => part.groupEdges(merge))
    new GraphImpl(vertices, replicatedVertexView.withEdges(newEdges))
  }
  ...
  override def aggregateMessagesWithActiveSet[A: ClassTag](
    ...
    val preAgg = view.edges.partitionsRDD
      .mapPartitions(_.flatMap { case (_, edgePartition) =>
        // Choose scan method
        val activeFraction =
        ...
        activeDirectionOpt match {
          case Some(EdgeDirection.Both) =>
            if (activeFraction < 0.8) {
              edgePartition
                .aggregateMessagesIndexScan(sendMsg, mergeMsg, tripletFields, EdgeActiveness.Both)
            } else {
              edgePartition
  ...
  override def outerJoinVertices[U: ClassTag, VD2: ClassTag](other: RDD[(VertexId, U)])(
      updateF: (VertexId, VD, Option[U]) => VD2)(implicit
      eq: VD =:= VD2 = null): Graph[VD2, ED] = {
    // The implicit parameter eq will be populated by the compiler if VD and VD2 are equal, and left
    // null if not
    if (eq != null) {
...
object GraphImpl {
   ...
   * vertices. The VertexRDD must already be set up for efficient joins with the EdgeRDD by
   * calling `VertexRDD.withEdges` or an appropriate VertexRDD constructor.
   */
