...
class GraphOpsSuite extends AnyFunSuite with LocalSparkContext {
  test("joinVertices") {
    withSpark { sc =>
      ...
      val g: Graph[String, String] = Graph[String, String](vertices, edges)

      val tbl = sc.parallelize(Seq[(VertexId, Int)]((1, 10), (2, 20)))
      val g1 = g.joinVertices(tbl) { (_: VertexId, attr: String, u: Int) => attr + u }
  ...
  test("collectNeighborIds") {
    withSpark { sc =>
      ...
      nbrs.collect().foreach { case (_, nbrs) => assert(nbrs.length === 2) }
  ...
  test("removeSelfEdges") {
    withSpark { sc =>
      ...
      val graph = Graph.fromEdgeTuples(sc.parallelize(edgeArray.toIndexedSeq), 1)
      val canonicalizedEdges = graph
        .removeSelfEdges()
        .edges
      ...
      val filteredGraph = graph
        .filter(
          graph => {
            ...
            graph.outerJoinVertices(degrees) { (_, _, deg) => deg.getOrElse(0) }
          },
  ...
  test("convertToCanonicalEdges") {
    withSpark { sc =>
      ...
      val g: Graph[String, Int] = Graph[String, Int](vertices, edges)
  ...
  test("collectEdgesCycleDirectionOut") {
    withSpark { sc =>
      ...
      edges.collect().foreach { case (_, edges) => assert(edges.length == 1) }
  ...
  test("collectEdgesCycleDirectionEither") {
    withSpark { sc =>
      ...
      assert(edges.count() == 100)
      edges.collect().foreach { case (_, edges) => assert(edges.length == 2) }
      edges.collect().foreach { case (vid, edges) =>
        val s = edges.toSet
