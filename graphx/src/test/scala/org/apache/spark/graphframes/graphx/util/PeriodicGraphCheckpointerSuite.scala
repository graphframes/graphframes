...
class PeriodicGraphCheckpointerSuite extends AnyFunSuite with LocalSparkContext {

  import PeriodicGraphCheckpointerSuite.*
...
private object PeriodicGraphCheckpointerSuite extends Assertions {
   ...
   * @param gIndex
   *   Index of graph in order inserted into checkpointer (from 1).
   * @param iteration
   *   Total number of graphs inserted into checkpointer.
   */
  def checkPersistence(graph: Graph[_, _], gIndex: Int, iteration: Int): Unit = {
    try {
      ...
      case _: AssertionError =>
        throw new Exception(
          "PeriodicGraphCheckpointerSuite.checkPersistence failed with:\n" +
            s"\t gIndex = $gIndex\n" +
