...
private[graphx] object RoutingTablePartition {
   ...
   * partition references the vertex (src, dst, or both). The edge partition is encoded in the
   * lower 30 bits of the Int, and the position is encoded in the upper 2 bits of the Int.
   */
...
private[graphx] class RoutingTablePartition(
  ...
  def reverse: RoutingTablePartition = {
    new RoutingTablePartition(routingTable.map { case (vids, srcVids, dstVids) =>
