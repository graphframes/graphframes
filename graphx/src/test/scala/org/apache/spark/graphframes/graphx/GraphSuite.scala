...
class GraphSuite extends AnyFunSuite with LocalSparkContext {

  def starGraph(sc: SparkContext, n: Int): Graph[String, Int] = {
    Graph.fromEdgeTuples[String](
      sc.parallelize((1L to n.toLong).map(x => (0L: VertexId, x: VertexId)), 3),
      "v")
  }
  ...
  test("triplets") {
    withSpark { sc =>
      ...
      assert(
        star.triplets.map(et => (et.srcId, et.dstId, et.srcAttr, et.dstAttr)).collect().toSet
          === (1L to n.toLong).map(x => (0L: VertexId, x: VertexId, "v", "v")).toSet)
    }
  }
  ...
  test("partitionBy") {
    withSpark { sc =>
      ...
      // the same partition
      assert(
        nonemptyParts(mkGraph(canonicalEdges).partitionBy(CanonicalRandomVertexCut))
          .count() === 1)
      ...
      if (!verts.forall(id => partitionSets.count(_.contains(id)) <= bound)) {
        ...
        fail(
          ("Replication bound test failed for %d/%d vertices. " +
            ...
            .format(numFailures, n, failure, partitionSets.count(_.contains(failure)), bound))
      }
      // This should not be true for the default hash partitioning
      val partitionSetsUnpartitioned = graph.edges.partitionsRDD
        .mapPartitions { iter =>
          val part = iter.next()._2
          Iterator((part.iterator.flatMap(e => Iterator(e.srcId, e.dstId))).toSet)
        }
  ...
  test("mapVertices") {
    withSpark { sc =>
      ...
      val mappedVAttrs = star.mapVertices((_, attr) => attr + "2")
      assert(
        mappedVAttrs.vertices.collect().toSet === (0L to n.toLong)
          .map(x => (x: VertexId, "v2"))
          .toSet)
      // mapVertices changing type
      val mappedVAttrs2 = star.mapVertices((_, attr) => attr.length)
      assert(
        mappedVAttrs2.vertices.collect().toSet === (0L to n.toLong)
          .map(x => (x: VertexId, 1))
          .toSet)
    }
  }
  ...
  test("mapVertices changing type with same erased type") {
    withSpark { sc =>
      val vertices = sc.parallelize(
        Seq[(Long, Option[java.lang.Integer])]((1L, Some(1)), (2L, Some(2)), (3L, Some(3))))
      val edges = sc.parallelize(Seq(Edge(1L, 2L, 0), Edge(2L, 3L, 0), Edge(3L, 1L, 0)))
      val graph0 = Graph[Option[java.lang.Integer], Int](vertices, edges)
      // Trigger initial vertex replication
      graph0.triplets.foreach(_ => {})
      // Change type of replicated vertices, but preserve erased type
      val graph1 = graph0.mapVertices { case (_, integerOpt) =>
  ...
  test("mapEdges") {
    withSpark { sc =>
      ...
      assert(edges.toSet === (1 to n).map(x => Edge(0L, x.toLong, x)).toSet)
    }
  }
  ...
  test("mapTriplets") {
    withSpark { sc =>
      ...
      val star = starGraph(sc, n)
      assert(
        star.mapTriplets(et => et.srcAttr + et.dstAttr).edges.collect().toSet ===
          (1L to n).map(x => Edge(0L, x.toLong, "vv")).toSet)
    }
  }
  ...
  test("reverse") {
    withSpark { sc =>
      val n = 5
      val star = starGraph(sc, n)
      assert(star.reverse.outDegrees.collect().toSet === (1 to n).map(x => (x.toLong, 1)).toSet)
    }
  }
  ...
  test("reverse with join elimination") {
    withSpark { sc =>
      ...
      val graph = Graph[Int, Int](vertices, edges).reverse
  ...
  test("subgraph") {
    withSpark { sc =>
      ...
      val subgraph = star.subgraph(vpred = (vid, _) => vid % 2 == 0)
      ...
      assert(
        subgraph.edges.map(_.copy()).collect().toSet ===
          (2 to n by 2).map(x => Edge(0, x.toLong, 1)).toSet)
    }
  }
  ...
  test("mask") {
    withSpark { sc =>
      val n = 5
      val vertices = sc.parallelize((0 to n).map(x => (x.toLong, x)))
      val edges = sc.parallelize((1 to n).map(x => Edge(0L, x.toLong, x)))
      val graph: Graph[Int, Int] = Graph[Int, Int](vertices, edges).cache()

      val subgraph = graph
        .subgraph(e => e.dstId != 4L, (vid, _) => vid != 3L)
        .mapVertices((_, _) => -1)
        .mapEdges(_ => -1)
  ...
  test("groupEdges") {
    withSpark { sc =>
      ...
      val doubleStar = Graph.fromEdgeTuples(
        sc.parallelize((1 to n).flatMap(x => List((0L, x.toLong), (0L, x.toLong))), 1),
        "v")
      val star2 = doubleStar.groupEdges { (a, _) => a }
      assert(
        star2.edges.collect().toArray.sorted(Edge.lexicographicOrdering[Int]) ===
  ...
  test("aggregateMessages") {
    withSpark { sc =>
      ...
      val agg = starGraph(sc, n).aggregateMessages[String](
        ...
        _ + _,
        TripletFields.Src)
      assert(agg.collect().toSet === (1L to n.toLong).map(x => (x: VertexId, "v")).toSet)
    }
  }
  ...
  test("outerJoinVertices") {
    withSpark { sc =>
      ...
      val neighborDegreeSums = GraphXUtils
        ...
        .collect()
        .toSet
      assert(neighborDegreeSums === Set((0L, n)) ++ (1 to n).map(x => (x.toLong, 0)))
      // outerJoinVertices preserving type
      val messages = reverseStar.vertices.mapValues { (vid, _) => vid.toString }
      val newReverseStar =
        reverseStar.outerJoinVertices(messages) { (_, a, bOpt) => a + bOpt.getOrElse("") }
      assert(
  ...
  test("more edge partitions than vertex partitions") {
    withSpark { sc =>
      ...
      val graph = Graph[String, Int](verts, edges)
  ...
  test("non-default number of edge partitions") {
    ...
    try {
      val edges = sc.parallelize((1 to n).map(x => (x.toLong, 0L)), numEdgePartitions)
      val graph = Graph.fromEdgeTuples(edges, 1)
      val neighborAttrSums = GraphXUtils.mapReduceTriplets[Int, Int, Int](
        graph,
        et => Iterator((et.dstId, et.srcAttr)),
        _ + _)
      assert(neighborAttrSums.collect().toSet === Set((0L, n)))
    } finally {
      sc.stop()
    }
  }
  ...
  test("unpersist graph RDD") {
    withSpark { sc =>
      ...
      val g0 = Graph[String, Long](vert, edges)
